...
public class ProductServiceImp implements ProductService{
	...
	public ProductDto addProduct(ProductRequest request, List<MultipartFile> images) {
		...
		Category categoryExists=crepo.findByCategoryName(request.getCategoryName()).orElseThrow(()->new ProductException("Category Is Not Found!",HttpStatus.NOT_FOUND));
		
		SubCategory scexists=screpo.findBySubCategoryNameAndCategory(request.getSubCategoryName(), categoryExists).orElseThrow(()->new ProductException("SubCategory is Not Found!", HttpStatus.NOT_FOUND));
		
		Products p=mapper.map(request, Products.class);	
		p.setBrand(ifExists);
		p.setSubCategory(scexists);
		//p.setCategory(categoryExists);
		p=prepo.save(p);
		eservice.SendMailtoUser("abhisheknaik4001@gmail.com", p.getProductName()+" "+p.getDescription());
	...
	public ProductDto updateProduct(Integer productId,UpdateProductRequest request, List<MultipartFile> images) {
		...
		SubCategory sc=screpo.findBySubCategoryNameAndCategory(request.getSubCategoryName(), ifExistscate).orElseThrow(()->new ProductException("Sub Category is Not Found!", HttpStatus.NOT_FOUND));
